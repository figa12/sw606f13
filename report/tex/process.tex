...
We arranged a meeting with Tove Søby, so that we could present our idea and show her the paper prototype that we had made to demonstrate our idea.% The feedback we received was used throughout the project to design our game. 
...
The sliding background that works, however we discovered that it is too slow, we discussed whether or not we should use \ac{opengles} instead. 
...
\begin{itemize}
\item We decided to use \ac{opengles} $1.x$ instead of the newer $2.0$. We chose to use $1.x$ because it should be easier for OpenGL beginners. 
...
\item We implemented the Pictogram class.
\item We sent an e-mail to Tove Søby with some prototypes of how the game looks. She has replied with some improvements.
\end{itemize} 
...
\item There are hills in the background. They are made in sequences of four hills and are randomly picked throughout the game. 
\item There are clouds and a sun on the sky. The clouds are moving at a random speed and at a random height on the sky. 
\item There cows and trees on the hills. These come at random times throughout the game.
\item There is a train depot to indicate that the game has finished.
\item Pictograms are drawn on the station when the train begins to drive. This is done with \ac{opengles} since we had some issues animating the layouts to follow the train.  
\item The game customisation menu is finished, it is now possible to create/save/delete game configurations.
\end{itemize}
...
\section*{Week 15 \& 16 - 27/05/2013 - 04/06/2013}
